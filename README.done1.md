...
- move _compute_observations to humanoid_hhi, so we can move `_compute_task_obs_v7` to humanoid_hhi, 
then in `_compute_task_obs_v7` we can pass `key_pos` to red marker hooks

- merge humanoid.py and humanoid_hhi.py, HumanoidHHI will inherit BaseTask directly.
...
- what is the purpose of `def fetch_amp_obs_demo(self, num_samples)` in humanoid_hhi.py. Build the descriminator reward
...
- 1. **Shape-conditioned discriminator plumbing**
   ...
   * **Code record:** `humanoid_hhi.py` now exports `extras["amp_shape"] = self._betas_env`; `hhi_agent.py` stores `amp_shape` through rollout, dataset, replay, and minibatch assembly; `hhi_models.py` evaluates all three discriminator branches as `eval_disc(amp_obs, amp_shape)`, `eval_disc(amp_obs_replay, amp_shape_replay)`, and `eval_disc(amp_obs_demo, amp_shape_demo)`; `hhi_network_builder.py` builds a discriminator morphology conditioner and defines `eval_disc(self, amp_obs, amp_shape)`. ([GitHub][1])
...
3. **Policy observation redesign for PHC transfer, plus AMP/discriminator observation redesign**
   ...
   * **Code record:** `humanoid_hhi.py` now sets `self._num_task_obs = 24 * num_key_bodies * self._num_traj_samples` and then `self._num_obs += self._num_task_obs`, giving 945 total obs. The same file also enables DOF subsetting for AMP, computes `num_amp_joints = len(self.dof_subset) // 3`, and rebuilds `_num_amp_obs_per_step` from the reduced subset, producing the 196-per-step discriminator input used later as 1960 over 10 steps. ([GitHub][1])
...
5. **Manual PHC-3 transfer loader**
   ...
   * **Code record:** `hhi_agent.py` defaults to `hhi_models/phc_3_Humanoid.pth`, explicitly documents the `934 = 358 + 576` source geometry, remaps old PHC keys such as `a2c_network.pnn.actors.0.0.weight` into the current actor trunk, and loads with `strict=False` after filtering shape mismatches. ([GitHub][6])
...
7. **Training/config refinements after the baseline**
   ...
   * **Code record:** `hhi_humanoid.yaml` now sets the actor MLP activation to `silu`, and `humanoid_hhi.py` sets `self.power_coefficient = cfg["env"].get("power_coefficient", 0.00005)`. ([GitHub][10])
...
[1]: https://github.com/hansen1416/hhi/blob/main/ase/env/tasks/humanoid_hhi.py "hhi/ase/env/tasks/humanoid_hhi.py at main · hansen1416/hhi · GitHub"
[2]: https://github.com/hansen1416/hhi/commits/main/ase/env/tasks/humanoid_hhi.py "History for ase/env/tasks/humanoid_hhi.py - hansen1416/hhi · GitHub"
[3]: https://github.com/hansen1416/hhi/blob/main/ase/learning/hhi_network_builder.py "hhi/ase/learning/hhi_network_builder.py at main · hansen1416/hhi · GitHub"
...
[6]: https://github.com/hansen1416/hhi/blob/main/ase/learning/hhi_agent.py "hhi/ase/learning/hhi_agent.py at main · hansen1416/hhi · GitHub"
[7]: https://github.com/hansen1416/hhi/commits/main/ase/learning/hhi_agent.py "History for ase/learning/hhi_agent.py - hansen1416/hhi · GitHub"
...
[10]: https://github.com/hansen1416/hhi/blob/main/ase/data/cfg/train/rlg/hhi_humanoid.yaml "hhi/ase/data/cfg/train/rlg/hhi_humanoid.yaml at main · hansen1416/hhi · GitHub"
